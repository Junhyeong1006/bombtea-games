(() => {
  ...
  const translations = {
    ...
    'facts.multiplayer': 'Online multiplayer', 'facts.platform': 'Platform',
    'facts.release': 'Release date', 'facts.releaseDate': 'Planned for November',
    ...
    'video.heading': 'See the game in action', 'video.hint': 'Swipe or use the arrows to explore.',
    'video.loading': 'Loading video…', 'video.error': 'This video couldn’t play. Try another clip.',
    'video.note': 'Actual gameplay from our development build.',
    'video.sync-cast': 'Make the Runners copy your moves and corner them.',
    'video.sync-runner': 'Stay on your toes.',
    'video.bomb-pass': 'Pass the bomb to a friend!',
    'video.tagger-attack': 'Chase down the Runners and catch them.',
    'video.tagger-trouble': 'Mix things up with a variety of items.',
    'video.ghost-trouble': 'Even after elimination, keep playing as a Ghost until the round ends.',
    'video.island-map': 'Chase your friends across a beautiful map.',
    ...
    'outro.description': 'We’re planning a November release on Steam. We’ll post updates here as development continues.',
  ...
  const englishLabels = {
    ...
    'video.carousel': 'Gameplay videos', 'video.prev': 'Previous video', 'video.next': 'Next video', 'video.select': 'Choose a video',
    'video.select.sync-cast': 'Video 2: Synchronize movement', 'video.select.sync-runner': 'Video 3: Being synchronized',
    'video.select.bomb-pass': 'Video 4: Pass the bomb', 'video.select.tagger-attack': 'Video 5: The Tagger attacks',
    'video.select.tagger-trouble': 'Video 6: Trouble for the Tagger', 'video.select.ghost-trouble': 'Video 7: Ghost interference',
    'video.select.island-map': 'Video 1: Explore the island'
  };
  const englishAlts = {
